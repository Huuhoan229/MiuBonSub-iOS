    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ScrollView {
                ...
                .padding(.bottom, 120) // Fix overlap
            }
        }
...
struct VideoPlayerSheet: View {
    ...
    private func saveCurrentProgress() async {
        ...
        await model.saveProgress(series: selection.watchFolder, index: selection.episodeIndex, time: seconds)
    }
}
"""

# Replace VideosView
start_videos_view = content.find("struct VideosView: View {")
end_video_now_playing = content.find("struct VideoLibraryCard: View {") # We will replace up to here, removing old videos views

# Remove old VideoLibraryCard and InlineVideoPlayer
start_inline_player = content.find("struct InlineVideoPlayer: View {")
end_inline_player = content.find("struct PipelineView: View {")

if start_videos_view != -1 and end_video_now_playing != -1:
    content = content[:start_videos_view] + videos_view_new + "\n\n" + content[end_video_now_playing:]

if start_inline_player != -1 and end_inline_player != -1:
    # Also remove VideoLibraryCard
    start_video_library_card = content.find("struct VideoLibraryCard: View {")
    if start_video_library_card != -1:
        content = content[:start_video_library_card] + content[end_inline_player:]
